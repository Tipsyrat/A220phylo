...
# Load data
pMor <- read.csv("https://raw.githubusercontent.com/Tipsyrat/A220phylo/main/primateMorpho.csv")
pMor <- as.data.frame(pMor)
mor <- as.matrix(pMor[,-1])
row.names(mor) <- as.vector(pMor[,1])

# Shiny UI for selecting number of traits
# Define the UI for the application
ui <- fluidPage(
    titlePanel("Examine Primate Traits"),
    sidebarLayout(
        sidebarPanel(
            sliderInput("start_trait", 
                        "Select starting trait:", 
                        min = 1, 
                        max = ncol(mor), 
                        value = 1, 
                        step = 1),
            sliderInput("num_traits", 
                        "Select number of traits to examine:", 
                        min = 1, 
                        max = ncol(mor), 
                        value = 21, 
                        step = 1)
...
# Shiny Server for rendering selected traits
server <- function(input, output) {
    output$traits_table <- renderTable({
        start <- input$start_trait
        end <- min(start + input$num_traits - 1, ncol(mor)) # Ensure we don't go beyond the number of columns
        mor[, start:end, drop = FALSE]
    })
}
...
knitr::include_graphics("pri_skull.jpg")
```
...
```{r, echo=FALSE,fig.align='center'}
knitr::include_graphics("convergence.jpg")
```
...
```{r, echo=FALSE,fig.align='center'}
knitr::include_graphics("MamMT.jpg")
```
...
bab_DNA <- bab(DNAchar)
bab_DNA_cons <- root(consensus(bab_DNA), outgroup = "Tarsier",
                    resolve.root = TRUE, edgelabel = TRUE)
